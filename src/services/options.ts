...
export type DecoStyleRow = Database["public"]["Tables"]["decoration_styles"]["Row"];

type FlavorJunction = { product_id: string; flavor_option_id: string };
type DecoJunction = { product_id: string; decoration_style_id: string };

export type ProductWithDetails = ProductRow & {
  sizes: SizeRow[];
  allowed_flavors: FlavorRow[];
  allowed_toppings: FlavorRow[];
  allowed_decoration_styles: DecoStyleRow[];
};
...
async function buildProductDetails(onlyActive: boolean): Promise<ProductWithDetails[]> {
  ...
  const [
    ...
    supabase.from("product_decoration_styles").select("product_id, decoration_style_id"),
  ]);

  const allFlavors: FlavorRow[] = flavors ?? [];
  const activeFlavors = onlyActive ? allFlavors.filter((f) => f.is_active) : allFlavors;

  const allDecoStyles: DecoStyleRow[] = decoStyles ?? [];
  const activeDecoStyles = onlyActive ? allDecoStyles.filter((d) => d.is_active) : allDecoStyles;

  const allSizes: SizeRow[] = sizes ?? [];
  const activeSizes = onlyActive ? allSizes.filter((s) => s.is_active) : allSizes;

  const flavorJunction: FlavorJunction[] = pFlavors ?? [];
  const toppingJunction: FlavorJunction[] = pToppings ?? [];
  const decoJunction: DecoJunction[] = pDecoStyles ?? [];
  ...
  return (products ?? []).map((product) => {
    ...
    const decoIds = decoJunction
      .filter((j) => j.product_id === product.id)
      .map((j) => j.decoration_style_id);
    ...
    return { ...product, sizes: productSizes, allowed_flavors, allowed_toppings, allowed_decoration_styles };
  });
}

export async function getPublicCatalog(): Promise<PublicCatalog> {
  const supabase = createSupabaseServerClient();
  const [products, { data: flavors }, { data: decoStyles }] = await Promise.all([
    buildProductDetails(true),
    supabase.from("flavor_options").select("*").eq("is_active", true).order("sort_order"),
    supabase.from("decoration_styles").select("*").eq("is_active", true).order("sort_order"),
  ]);
  return { products, allFlavors: flavors ?? [], allDecoStyles: decoStyles ?? [] };
}
